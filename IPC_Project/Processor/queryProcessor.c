...
int AllSubReportFunc(const void* _key, void* _value, void* _context)
{
	...
	if (NULL == (fp = fopen("All_Sub_Report.txt", "a+"))) 
	{
		return -1;
	}
...
static int ImsiRetrieve(Aggregator* _agg, char* _key)
{
	void* pValue;
	Billing* billing;
...
int CGF(Aggregator* _agg, char* _key, char _funcNum)
{
...
/*	void* msgToReceive;*/
	...
	while (1)
	{
		...
		CGF(agg, CGFtoReceive.m_query, CGFtoReceive.m_funcNum);
	}
